...
```
medguide-ai/
├── app.py                  # Single Streamlit entrypoint — the entire "frontend"
├── agent/                  # All agent/orchestration logic lives here, isolated from UI code
│   ├── __init__.py
│   ├── graph.py             # LangGraph StateGraph — the "brain"
│   ├── tools.py             # Retriever + validator — the agent's callable tools
│   └── prompts.py           # All prompt text centralized — easy to tune without touching logic
...
│   ├── lab_reference.csv
│   └── synthetic_labs.csv
├── docs/                    # Design artifacts + future screenshots
│   ├── ARCHITECTURE.md
│   ├── SCHEMA.md
│   ├── API.md
│   ├── UI-WIREFRAMES.md
...
├── chroma_store/            # Generated by ingestion; gitignored until Day 9, then committed
